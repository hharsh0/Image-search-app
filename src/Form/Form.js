...
function Form() {
    ...
    const changePhoto = ()=>{
        axios.get(`https://api.unsplash.com/search/photos?page=1&query=${img}&client_id=${Access_Key}`)
        .then((response)=>{
  ...
  return (
    <>
    <div className='form'>
        <input  className="input" value={img} type='text' placeholder='Search anything...' 
        onChange={(e)=>setImg(e.target.value)}></input>
        <button className='btn' onClick={changePhoto} type="submit">Search</button>
    </div>
    { show === 1 &&
    <div class="imgs">
        <img class="a" src={res} />
